...
def module_checks(skip_unless_sparse_vectors_supported):
    pass


def _test_insert_and_fetch(cursor, vector, column_name, expected_typecode):
    """
...
def _test_insert_and_fetch_sparse(
    cursor, vector, column_name, expected_typecode
):
    """
    ...
    cursor.connection.commit()
    cursor.execute(f"select {column_name} from TestSparseVectors")
    (fetched_value,) = cursor.fetchone()
    ...
    assert fetched_value.values == expected_value
    assert fetched_value.indices == expected_indices
    assert fetched_value.num_dimensions == expected_num_dimensions
...
def test_7700(cursor):
    ...
    (fetched_value,) = cursor.fetchone()
    assert isinstance(fetched_value, oracledb.SparseVector)
    assert fetched_value.num_dimensions == vector.num_dimensions
    assert fetched_value.indices == vector.indices
    assert fetched_value.values == vector.values


def test_7701(cursor):
...
def test_7702(cursor):
    ...
    assert fetched_value.indices == vector.indices
    assert fetched_value.num_dimensions == vector.num_dimensions
    assert fetched_value.values.typecode == "d"
    assert isinstance(fetched_value, oracledb.SparseVector)


def test_7703(cursor):
    ...
    assert fetched_value.indices == vector.indices
    assert fetched_value.num_dimensions == vector.num_dimensions
    assert fetched_value.values.typecode == "b"
    assert isinstance(fetched_value, oracledb.SparseVector)


def test_7704(cursor):
...
def test_7711(cursor):
    ...
    cursor.execute("select * from TestSparseVectors")
...
def test_7718(conn, cursor):
    ...
    assert vector.values == value.values
    assert vector.indices == value.indices
    assert vector.num_dimensions == value.num_dimensions


def test_7719(conn, cursor):
    "7719 - test handling of NULL vector value"
    cursor.execute("delete from TestSparseVectors")
    cursor.execute("insert into TestSparseVectors (IntCol) values (1)")
    conn.commit()
    cursor.execute("select SparseVectorFlexTypeCol from TestSparseVectors")
    (fetched_value,) = cursor.fetchone()
    assert fetched_value is None


def test_7720(cursor, test_env):
    ...
    for dim in [30, 70, 255, 256, 65534, 65535]:
        for typ in ["f", "d", "b"]:
            ...
            _test_insert_and_fetch_sparse(
                cursor, value, "SparseVectorFlexAllCol", typ
            )


def test_7723(cursor):
    ...
    for dim in [30, 255, 256, 257, 32768, 65535]:
        for source_type in ["f", "d", "b"]:
            for target_type in ["f", "d", "b"]:
                ...
                _test_insert_and_fetch_sparse(
                    cursor, value, f"Sparse{target_col}", target_type
                )


def test_7725(cursor, test_env):
    ...
    with test_env.assert_raises_full_code("ORA-51805", "ORA-51831"):
        cursor.execute("select :1 from dual", [value])


def test_7726(cursor):
...
def test_7727(cursor):
    ...
    assert fetched_vector.indices == vector.indices
    assert fetched_vector.values == vector.values
    assert fetched_vector.num_dimensions == vector.num_dimensions


def test_7728(cursor):
    ...
    assert fetched_vector1.values == vector.values
    assert fetched_vector1.indices == vector.indices
    assert fetched_vector1.num_dimensions == vector.num_dimensions
    assert fetched_vector2.values == array.array("f", [2.0, 1.0, 4.0])
    assert fetched_vector2.indices == array.array("I", [3, 9, 11])
    assert fetched_vector2.num_dimensions == 16


def test_7729(cursor):
    ...
    assert fetched_vector1.num_dimensions == 3
    assert fetched_vector2.values == vector.values
    assert fetched_vector2.indices == vector.indices
...
def test_7731(cursor):
    ...
    assert vector.num_dimensions == 4
...
def test_7732():
    ...
    assert value.num_dimensions == 10
...
def test_7735(cursor):
    ...
    for vector in [dense_vector, sparse_vector]:
        for column_name in column_names:
            _fetch_with_vector(cursor, vector, column_name, 16, "INT8", "b")
            _fetch_with_vector(cursor, vector, column_name, 16, "FLOAT32", "f")
            _fetch_with_vector(cursor, vector, column_name, 16, "FLOAT64", "d")
    ...
    for vector in [dense_vector, sparse_vector]:
        for column_name in column_names[3:]:
            _fetch_with_vector(cursor, vector, column_name, 2, "INT8", "b")
            _fetch_with_vector(cursor, vector, column_name, 2, "FLOAT32", "f")
            _fetch_with_vector(cursor, vector, column_name, 2, "FLOAT64", "d")


def test_7736(cursor):
