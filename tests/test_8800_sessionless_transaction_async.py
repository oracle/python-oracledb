...
class TestCase(test_env.BaseAsyncTestCase):
    ...
    async def test_8801(self):
        ...
        self.assertEqual(len(await self.cursor.fetchall()), 2)

    async def test_8802(self):
    ...
    async def test_8804(self):
        ...
        async with test_env.get_connection_async() as conn:
            ...
            self.assertEqual(await cursor.fetchall(), [(1, "updated")])

    async def test_8805(self):
    ...
    async def test_8807(self):
        ...
        transaction_id = await self.conn.begin_sessionless_transaction()
        await self.cursor.execute(
            """
            insert into TestTempTable (IntCol, StringCol1)
            values (:1, :2)
    ...
    async def test_8808(self):
        ...
        with self.assertRaisesFullCode("DPY-3035"):
            await self.conn.resume_sessionless_transaction("wrong_id")

        # try to resume before suspend
        with self.assertRaisesFullCode("DPY-3035"):
            await self.conn.resume_sessionless_transaction(transaction_id)

        # suspend and resume correctly
        await self.conn.suspend_sessionless_transaction()
        async with test_env.get_connection_async() as conn:
            await conn.resume_sessionless_transaction(transaction_id)

    async def test_8809(self):
        "8809 - test getting transaction ID of active sessionless transaction"
        transaction_id = await self.conn.begin_sessionless_transaction()
        await self.cursor.execute(
            "select dbms_transaction.get_transaction_id()"
        )
        (server_transaction_id,) = await self.cursor.fetchone()
        self.assertEqual(server_transaction_id, transaction_id.hex().upper())
        await self.conn.commit()

    async def test_8810(self):
        "8810 - test auto-generated transaction ID uniqueness"

        # start first transaction
        transaction_id_1 = await self.conn.begin_sessionless_transaction()
        await self.conn.suspend_sessionless_transaction()
        ...
        # cleanup
        await self.conn.resume_sessionless_transaction(transaction_id_1)
        await self.conn.rollback()

    async def test_8811(self):
        ...
        async with pool.acquire() as conn:
            ...
            self.assertEqual(await cursor.fetchall(), data)

        await pool.close()

    async def test_8812(self):
        ...
        await self.conn.begin_sessionless_transaction(long_transaction_id)
        await self.cursor.execute(
            """
            insert into TestTempTable (IntCol, StringCol1)
            values (:1, :2)
        ...
        await self.conn.begin_sessionless_transaction(special_transaction_id)
        await self.cursor.execute(
            """
            insert into TestTempTable (IntCol, StringCol1)
            values (:1, :2)
    ...
    async def test_8815(self):
        "8815 - transaction behaviour with DDL operations"

        # create temp table
        temp_table_name = "temp_test_8815"
        await self.cursor.execute(f"drop table if exists {temp_table_name}")
        await self.cursor.execute(
            f"""
        ...
        # implicit commit
        await self.conn.begin_sessionless_transaction()
        await self.cursor.execute(
        ...
        await self.cursor.execute(f"select * from {temp_table_name}")
        self.assertEqual(await self.cursor.fetchall(), [local_data])

        # drop temp table
        await self.cursor.execute(f"drop table {temp_table_name} purge")
