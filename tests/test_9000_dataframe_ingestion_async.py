...
class TestCase(test_env.BaseAsyncTestCase):
    ...
    async def test_9005(self):
        ...
        df = pyarrow.table(arrays, names)
        await self.cursor.execute("truncate table TestBooleans")
        await self.cursor.executemany(
            """
    ...
    async def test_9017(self):
        ...
        df = pyarrow.table(arrays, names)
        await self.cursor.execute("delete from TestSparseVectors")
    ...
    async def test_9018(self):
        ...
        df = pyarrow.table(arrays, names)
        await self.cursor.execute("delete from TestVectors")
        column_names = ",".join(names)
        bind_names = ",".join(f":{i + 1}" for i in range(len(names)))
        await self.cursor.executemany(
            f"""
